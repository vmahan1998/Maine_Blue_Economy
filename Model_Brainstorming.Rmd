...
for (species_name in species_list) {
  ...
  blue_economy_data <- df_species %>%
    dplyr::select(Total_Value, Avg_Market_Price, Total_Weight_kg, Fishery_Exports, Mean_Sea_Level_m)
...
# Subset model forecast data for selected species and baseline scenario
...
historic_data <- interpolated_data %>%
  ...
  group_by(Year, Species, Data_Source) %>%
  summarize(across(c(Total_Weight_kg, Avg_Market_Price, Total_Value), mean, na.rm = TRUE), .groups = "drop") %>%
  distinct(Year, Species, Data_Source, .keep_all = TRUE)  # Ensure uniqueness
...
combined_data <- bind_rows(historic_data, model_data) %>%
  arrange(Species, Year, Data_Source) %>%
  distinct(Year, Species, Data_Source, .keep_all = TRUE)  # Ensure no duplicates

# Generate half-year intervals for smoother plotting
full_years <- seq(floor(min(combined_data$Year)), ceiling(max(combined_data$Year)), by = 0.5)
expanded_data <- expand.grid(Year = full_years, Species = unique(combined_data$Species))

# Merge and interpolate missing values
combined_data <- expanded_data %>%
  left_join(combined_data, by = c("Year", "Species")) %>%
  group_by(Species) %>%
  mutate(
...
# Set "Data_Source": anything < 2020 is "Historic", anything >= 2020 is "Projected"
combined_data <- combined_data %>%
  mutate(
    Data_Source = case_when(
      Year < 2021 ~ "Historic",
      Year >= 2021 ~ "Projected",
      TRUE ~ Data_Source  # Preserve existing values if already set
    )
  )

# Interpolate missing values for numeric variables
combined_data <- combined_data %>%
  group_by(Species) %>%
  mutate(
    Total_Weight_kg = na.approx(Total_Weight_kg, na.rm = FALSE, rule = 2),
    Avg_Market_Price = na.approx(Avg_Market_Price, na.rm = FALSE, rule = 2),
    Total_Value = na.approx(Total_Value, na.rm = FALSE, rule = 2)
  ) %>%
  ungroup()
...
- Market Price
- could be lots of new landings available at increased moderate price
- fewer new landings at higher price
- correlation of change in landings and market price?
