...
for (species_name in species_list) {
  ...
  landings_data <- df_species %>%
    dplyr::select(Total_Weight_kg, Mean_Sea_Level_m, Portland_Annual_Precip, 
                  ...
                  Augusta_Annual_Temp, Annual_NAO, State_Population)
  ...
  rf_landings <- randomForest(Total_Weight_kg ~ ., data = train_landings, ntree = 1000)
  
  # 🔹 Train Consumption Model
  consumption_data <- df_species %>%
    dplyr::select(Total..Pounds., Total_Weight_kg, Civilian.Population..Millions., State_Population)
  
  trainIndex <- createDataPartition(consumption_data$Total..Pounds., p = 0.8, list = FALSE)
  train_consumption <- consumption_data[trainIndex, ]
  test_consumption <- consumption_data[-trainIndex, ]
  
  rf_consumption <- randomForest(Total..Pounds. ~ ., data = consumption_data, ntree = 1000)
  
  # 🔹 Train Exports Model
  exports_data <- df_species %>%
    dplyr::select(Fishery_Exports, Total_Weight_kg, Total..Pounds.)

  rf_exports <- randomForest(Fishery_Exports ~ ., data = exports_data, ntree = 1000)

  trainIndex <- createDataPartition(exports_data$Fishery_Exports, p = 0.8, list = FALSE)
  ...
  market_value_data <- df_species %>%
    dplyr::select(Avg_Market_Price, GDP..Billions.of.US..., Annual...Change, 
                  Per.Capita..US..., Total_Weight_kg, Total..Pounds.)
  
  trainIndex <- createDataPartition(market_value_data$Avg_Market_Price, p = 0.8, list = FALSE)
  train_market_value <- market_value_data[trainIndex, ]
  test_market_value <- market_value_data[-trainIndex, ]
  
  rf_market_value <- randomForest(Avg_Market_Price ~ ., data = market_value_data, ntree = 1000)

  # 🔹 Train Blue Economy Model
  blue_economy_data <- df_species %>%
    dplyr::select(Total_Value, Avg_Market_Price, Total_Weight_kg)

  rf_blue_economy <- randomForest(Total_Value ~ ., data = blue_economy_data, ntree = 1000)
