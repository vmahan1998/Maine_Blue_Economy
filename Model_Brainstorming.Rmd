...
gdp_data_cleaned <- GDP %>%
  mutate(Year = year(mdy(Date))) %>%  # Convert date and extract year
  filter(Year >= 1950 & Year <= 2020) %>%  # Keep only years 1950-2020
  select(Year, GDP..Billions.of.US..., Per.Capita..US..., Annual...Change)  # Keep relevant columns
...
merged_data <- merged_data %>%
  left_join(portland_temp %>% select(Year, Portland_Annual_Temp), by = "Year") %>%
  left_join(Portland_precip %>% select(Year, Portland_Annual_Precip), by = "Year")
...
# Merge temperature and precipitation data
merged_data <- merged_data %>%
  left_join(belfast_temp %>% select(Year, Belfast_Annual_Temp), by = "Year") %>%
  left_join(belfast_precip %>% select(Year, Belfast_Annual_Precip), by = "Year")
...
# Merge temperature and precipitation data
merged_data <- merged_data %>%
  left_join(moosehead_temp %>% select(Year, Moosehead_Annual_Temp), by = "Year") %>%
  left_join(moosehead_precip %>% select(Year, Moosehead_Annual_Precip), by = "Year")
...
# Merge temperature and precipitation data
merged_data <- merged_data %>%
  left_join(gardiner_temp %>% select(Year, Gardiner_Annual_Temp), by = "Year") %>%
  left_join(gardiner_precip %>% select(Year, Gardiner_Annual_Precip), by = "Year")
...
# Merge temperature and precipitation data
merged_data <- merged_data %>%
  left_join(augusta_temp %>% select(Year, Augusta_Annual_Temp), by = "Year") %>%
  left_join(augusta_precip %>% select(Year, Augusta_Annual_Precip), by = "Year")
...
low_slr <- baseline_sea_levels + (0.44 * ((future_years - 2025) / (2100 - 2025))^2)  # SSP1-2.6 (~0.44m by 2100)
moderate_slr <- baseline_sea_levels + (0.60 * ((future_years - 2025) / (2100 - 2025))^2)  # SSP2-4.5 (~0.60m by 2100)
high_slr <- baseline_sea_levels + (1.0 * ((future_years - 2025) / (2100 - 2025))^2)  # SSP5-8.5 (~1.0m by 2100)
...
low_temp <- baseline_temp + (1.8 * ((future_years - 2025) / (2100 - 2025)))  # SSP1-2.6 (~1.8°C increase by 2100)
moderate_temp <- baseline_temp + (2.7 * ((future_years - 2025) / (2100 - 2025)))  # SSP2-4.5 (~2.7°C increase by 2100)
high_temp <- baseline_temp + (4.4 * ((future_years - 2025) / (2100 - 2025)))  # SSP5-8.5 (~4.4°C increase by 2100)
...
future_scenarios <- list(
  ...
  # Fishing Policy Changes
  "Fishing_Conservative" = data.frame(
    Year = future_years, 
    Mean_Sea_Level_m = baseline_sea_levels, 
    ENSO_Phase = enso_scenarios$ENSO_2yr, 
...
for (species_name in species_list) {
  ...
  species_data <- interpolated_data %>%
    ...
    select(Year, Total_Weight_kg, Total_Value, Mean_Sea_Level_m, Annual_Temp, ENSO_Phase, Annual_NAO) %>%
    arrange(Year)
  ...
  species_data <- species_data %>%
    mutate(across(c(Total_Weight_kg, Total_Value, Mean_Sea_Level_m, Annual_Temp, Annual_NAO), ~ zoo::na.approx(., na.rm = FALSE, rule = 2)))
  ...
  # Fit ARIMA models with external regressors (SLR, Temp, ENSO, NAO)
  fit_weight <- auto.arima(ts_weight, xreg = cbind(species_data$Mean_Sea_Level_m, species_data$Annual_Temp, species_data$ENSO_Phase_Num, species_data$Annual_NAO), seasonal = TRUE, stepwise = FALSE, approximation = FALSE)
  fit_value <- auto.arima(ts_value, xreg = cbind(species_data$Mean_Sea_Level_m, species_data$Annual_Temp, species_data$ENSO_Phase_Num, species_data$Annual_NAO), seasonal = TRUE, stepwise = FALSE, approximation = FALSE)
  ...
  for (scenario_name in names(future_scenarios)) {
    ...
    forecast_weight <- forecast(fit_weight, xreg = cbind(scenario_data$Mean_Sea_Level_m, scenario_data$Annual_Temp, scenario_data$ENSO_Phase_Num, scenario_data$Annual_NAO), h = length(future_years))
    forecast_value <- forecast(fit_value, xreg = cbind(scenario_data$Mean_Sea_Level_m, scenario_data$Annual_Temp, scenario_data$ENSO_Phase_Num, scenario_data$Annual_NAO), h = length(future_years))
...
base_output_dir <- "C:/Users/RDEL1VMM/Desktop/current projects/Maine_Blue_Economy/Clean Data/County_Plots"
...
for (county_name in county_list) {
  ...
  # Define county-specific output directory
  county_dir <- paste0(base_output_dir, "/", gsub(" ", "_", county_name))
  
  # Ensure county directory exists
  ...
  fit_lobster_pounds <- auto.arima(ts_lobster_pounds, xreg = cbind(county_data$Mean_Sea_Level_m, county_data$Annual_Temp), seasonal = TRUE, stepwise = FALSE, approximation = FALSE)
  fit_lobster_value <- auto.arima(ts_lobster_value, xreg = cbind(county_data$Mean_Sea_Level_m, county_data$Annual_Temp), seasonal = TRUE, stepwise = FALSE, approximation = FALSE)
...
for (county_name in county_list) {
  ...
  pounds_plot_filename <- paste0(county_dir, "/", gsub(" ", "_", county_name), "_Lobster_Pounds.png")
  ...
  value_plot_filename <- paste0(county_dir, "/", gsub(" ", "_", county_name), "_Lobster_Value.png")
  ggsave(value_plot_filename, plot = value_plot, width = 8, height = 6, dpi = 300)

  # Save county-specific results as CSV
  county_csv_filename <- paste0(county_dir, "/", gsub(" ", "_", county_name), "_Forecast_Results.csv")
  write.csv(county_data, county_csv_filename, row.names = FALSE)

  cat("Saved plots and results for:", county_name, "\n")
}

# Save final dataset for all counties
