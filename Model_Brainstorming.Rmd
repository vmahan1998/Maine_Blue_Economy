...
for (species_name in species_list) {
  ...
  landings_data <- df_species %>%
    dplyr::select(Total_Weight_kg, Mean_Sea_Level_m, Portland_Annual_Precip, 
                  Portland_Annual_Temp, Belfast_Annual_Precip, Belfast_Annual_Temp, 
                  Moosehead_Annual_Precip, Moosehead_Annual_Temp, Gardiner_Annual_Precip, 
                  Gardiner_Annual_Temp, Augusta_Annual_Precip, Augusta_Annual_Temp, 
                  Annual_NAO, State_Population)
  ...
  consumption_data <- df_species %>%
    dplyr::select(Total..Pounds., Total_Weight_kg, Civilian.Population..Millions., 
                  Cured..Pounds., Canned..Pounds., Fresh.And.Frozen..Pounds.)
  
  rf_consumption <- randomForest(Total..Pounds. ~ ., data = consumption_data, ntree = 500)

  # 🔹 Train Exports Model
  exports_data <- df_species %>%
    dplyr::select(Fishery_Exports, Total_Weight_kg, Total..Pounds.)

  rf_exports <- randomForest(Fishery_Exports ~ ., data = exports_data, ntree = 500)

  # 🔹 Train Market Value Model
  market_value_data <- df_species %>%
    dplyr::select(Avg_Market_Price, GDP..Billions.of.US..., Annual...Change, 
                  Per.Capita..US..., Total_Weight_kg, Total..Pounds., Fishery_Exports)
  ...
  blue_economy_data <- df_species %>%
    dplyr::select(Total_Value, Avg_Market_Price, Total_Weight_kg, Fishery_Exports)

  rf_blue_economy <- randomForest(Total_Value ~ ., data = blue_economy_data, ntree = 500)
...
```{r}
# Define base directory where results are stored
base_output_dir <- "C:/Users/RDEL1VMM/Desktop/current projects/Maine_Blue_Economy/Clean Data/Plots"
...
for (species_dir in species_dirs) {
  ...
  # Identify CSV file
  csv_file <- list.files(species_dir, pattern = "_Forecasts.csv$", full.names = TRUE)
...
# 🔹 Compute Maine’s Net Blue Economy Value (Total_Value summed across species)
maine_blue_economy <- combined_results %>%
...
# Save Maine's Net Blue Economy Value
maine_blue_economy_csv <- file.path(base_output_dir, "Maine_Net_Blue_Economy.csv")
write.csv(maine_blue_economy, maine_blue_economy_csv, row.names = FALSE)
cat("Saved Maine's Net Blue Economy Potential:", maine_blue_economy_csv, "\n")

# Aggregate US Exports and Consumption per Year
...
aggregated_csv_file <- file.path(base_output_dir, "Aggregated_Exports_Consumption.csv")
write.csv(aggregated_results, aggregated_csv_file, row.names = FALSE)
cat("Saved aggregated results:", aggregated_csv_file, "\n")
...
# 🔹 Plot Maine's Net Blue Economy Value
maine_blue_economy_plot <- ggplot(maine_blue_economy, aes(x = Year, y = Net_Blue_Economy_Value, color = Scenario)) +
  geom_line(size = 1.2) +
  ggtitle("Projected Net Blue Economy Potential for Maine") +
  xlab("Year") + ylab("Total Value ($)") +
  theme_minimal()

maine_blue_economy_plot_file <- file.path(base_output_dir, "Maine_Net_Blue_Economy_Projection.png")
ggsave(maine_blue_economy_plot_file, plot = maine_blue_economy_plot, width = 8, height = 5)
cat("Saved Maine Net Blue Economy plot:", maine_blue_economy_plot_file, "\n")

# US Exports Projection
export_plot <- ggplot(aggregated_results, aes(x = Year, y = Mean_Exports, color = Scenario)) +
  geom_line(size = 1.2) +
  ggtitle("Projected US Fishery Exports") +
  xlab("Year") + ylab("Mean Exports ($)") +
  theme_minimal()

export_plot_file <- file.path(base_output_dir, "Aggregated_Exports_Projection.png")
ggsave(export_plot_file, plot = export_plot, width = 8, height = 5)
cat("Saved export projection plot:", export_plot_file, "\n")

# US Consumption Projection
consumption_plot <- ggplot(aggregated_results, aes(x = Year, y = Mean_Consumption, color = Scenario)) +
  geom_line(size = 1.2) +
  ggtitle("Projected US Fishery Consumption") +
  xlab("Year") + ylab("Mean Consumption (Pounds)") +
  theme_minimal()

consumption_plot_file <- file.path(base_output_dir, "Aggregated_Consumption_Projection.png")
ggsave(consumption_plot_file, plot = consumption_plot, width = 8, height = 5)
cat("Saved consumption projection plot:", consumption_plot_file, "\n")
```
...
for (species_dir in species_dirs) {
  ...
  # Debug: Print the species directory being processed
  cat("Processing directory:", species_dir, "\n")
  
  # Identify CSV file
  csv_file <- list.files(species_dir, pattern = "Forecasts.csv$", full.names = TRUE)
  
  # Debug: Print the detected CSV file
  if (length(csv_file) == 0) {
    cat("No Forecasts.csv found in", species_dir, "\n")
    next  # Skip if no CSV file exists
  } else {
  ...
  # Read the forecasted data
  species_results <- tryCatch(
    {
      read.csv(csv_file)
    },
    error = function(e) {
      cat("Error reading:", csv_file, "\nError message:", e$message, "\n")
      return(NULL)  # Skip if there's an error
    }
  )
  
  if (is.null(species_results)) {
    next  # Skip this iteration if the file couldn't be read
  }
  
  # Extract species name from directory
  species_name <- basename(species_dir)
  
  # Debug: Print species name
...
# 🔹 Identify the top species contributing to Maine's Blue Economy
species_total_value <- combined_results %>%
  group_by(Species) %>%
  summarise(Total_Economic_Contribution = sum(Predicted_Blue_Economy_Value, na.rm = TRUE)) %>%
  arrange(desc(Total_Economic_Contribution))

# Save the ranked list of species contributions
species_contribution_csv <- file.path(base_output_dir, "Top_Species_Economic_Contribution.csv")
write.csv(species_total_value, species_contribution_csv, row.names = FALSE)
cat("Saved species economic contribution ranking:", species_contribution_csv, "\n")

# 🔹 Identify top species contributing per year
top_species_by_year <- combined_results %>%
  group_by(Year, Species) %>%
  summarise(Annual_Contribution = sum(Predicted_Blue_Economy_Value, na.rm = TRUE)) %>%
  arrange(Year, desc(Annual_Contribution))

# Save yearly ranking of species contributions
species_yearly_contribution_csv <- file.path(base_output_dir, "Top_Species_Economic_Contribution_Per_Year.csv")
write.csv(top_species_by_year, species_yearly_contribution_csv, row.names = FALSE)
cat("Saved top species economic contribution per year:", species_yearly_contribution_csv, "\n")

# 🔹 Compute the percentage change in contribution over time
species_trends <- combined_results %>%
  group_by(Species, Year) %>%
  summarise(Total_Contribution = sum(Predicted_Blue_Economy_Value, na.rm = TRUE)) %>%
  arrange(Species, Year) %>%
  group_by(Species) %>%
  mutate(Percent_Change = (Total_Contribution - lag(Total_Contribution)) / lag(Total_Contribution) * 100)

# Save species trends over time
species_trends_csv <- file.path(base_output_dir, "Species_Economic_Trend.csv")
write.csv(species_trends, species_trends_csv, row.names = FALSE)
cat("Saved species economic trend over time:", species_trends_csv, "\n")

# 🔹 Identify Top 10 and Bottom 10 Species
species_total_contribution <- species_trends %>%
  group_by(Species) %>%
  summarise(Total_Contribution = sum(Total_Contribution, na.rm = TRUE)) %>%
  arrange(desc(Total_Contribution))

top_10_species <- species_total_contribution %>% head(10) %>% pull(Species)
bottom_10_species <- species_total_contribution %>% tail(10) %>% pull(Species)

top_10_trends <- species_trends %>% filter(Species %in% top_10_species)
bottom_10_trends <- species_trends %>% filter(Species %in% bottom_10_species)

# 🔹 Plot Top 10 Species
top_10_plot <- ggplot(top_10_trends, aes(x = Year, y = Total_Contribution, color = Species)) +
  geom_line(size = 1.2) +
  ggtitle("Top 10 Species Contribution to Maine's Blue Economy") +
  xlab("Year") + ylab("Total Value ($)") +
  theme_minimal()

top_10_plot_file <- file.path(base_output_dir, "Top_10_Species_Contribution.png")
ggsave(top_10_plot_file, plot = top_10_plot, width = 8, height = 5)
cat("Saved top 10 species contribution plot:", top_10_plot_file, "\n")

# 🔹 Plot Bottom 10 Species
bottom_10_plot <- ggplot(bottom_10_trends, aes(x = Year, y = Total_Contribution, color = Species)) +
  geom_line(size = 1.2) +
  ggtitle("Bottom 10 Species Contribution to Maine's Blue Economy") +
  xlab("Year") + ylab("Total Value ($)") +
  theme_minimal()

bottom_10_plot_file <- file.path(base_output_dir, "Bottom_10_Species_Contribution.png")
ggsave(bottom_10_plot_file, plot = bottom_10_plot, width = 8, height = 5)
cat("Saved bottom 10 species contribution plot:", bottom_10_plot_file, "\n")
```
